package fr.noobtopia.plugin.rank;

import fr.noobtopia.plugin.io.Folders;
import fr.noobtopia.plugin.permission.PermissionManager;
...
public class Rank {
    ...
    public static File getFolderOf(Player player){
        File folder = new File(Folders.rank, player.getName());
    ...
    public static List<Rank> getRanksOf(Player player){
        ...
        for(File file : getFolderOf(player).listFiles()){
            ranks.add(Rank.fromId(Integer.parseInt(file.getName())));
        }
